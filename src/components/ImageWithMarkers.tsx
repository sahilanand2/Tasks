...
import { Box, Button, MenuItem, Select, Typography, Modal, Paper, List, ListItem, ListItemText, Divider, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, IconButton, Grid, Tooltip } from '@mui/material';
...
const ImageWithMarkers: React.FC<ImageWithMarkersProps> = ({ tasks, userId, onClose }) => {
  ...
  const handleImageClick = (e: React.MouseEvent) => {
    if (selectedTaskId) {
      ...
      dispatch(addMarker(newMarker)); // Ensure addMarker is defined in tasksSlice
    } else {
  ...
  const handleDeleteMarker = () => {
    if (selectedMarker) {
      ...
      dispatch(removeMarker(selectedMarker)); // Ensure removeMarker is defined in tasksSlice
      handleClosePopup();
      handleCloseDeleteConfirm(); // Close the delete confirmation dialog
    }
  };
